...
export function getTableCellKey(props: TableCellProperties): string {
  ...
  try {
    ...
    if (!variantObj) {
      throw new Error(
        `Invalid variant ${variant} for position ${position} and type ${type}`
      );
    }
    const key = variantObj[state];
    if (!key) {
      throw new Error(
        `Invalid state ${state} for variant ${variant}, position ${position}, and type ${type}`
      );
    }
    return key;
  } catch (error) {
    ...
    throw new Error(
      `Could not find component key for table cell with properties: ${JSON.stringify(props)}`
    );
  }
}
...
export function createTableCellProperties(
  props: Partial<TableCellProperties>
): TableCellProperties {
  return {
